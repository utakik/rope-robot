// --- cam-sketch.js : camera + 4-bone rope (standalone, with Flip) ---
// version: 2025-10-12-standalone-cam+flip

const VERSION = "2025-10-12-standalone-cam+flip";
...
// 差分解析（軽量用の内部解像度）
const vw = 240, vh = 180;
const DIFF_THRESH = 60;        // 暗所なら 40〜50 も可

let ropes = [], swayLP = 0, prevDir = 1;
let cnv, video, startBtn, flipBtn;
let prevGfx;
let flipDir = false;           // ← 左右反転トグル
...
function setup(){
  ...
  startBtn.mousePressed(async ()=>{
    try{
      ...
      // 左右反転トグル（カメラ開始後に出す）
      flipBtn = createButton('Flip: OFF');
      flipBtn.position(12, 52);
...
function updateSwayFromCamera(){
  if (!video || !prevGfx) return;
  video.loadPixels();
  prevGfx.loadPixels();
  if (video.pixels.length === 0 || prevGfx.pixels.length === 0) return;
  ...
  for (let y=0; y<vh; y+=2){
    for (let x=0; x<vw; x+=2){
      ...
      if (d>DIFF_THRESH){ sum+=d; sumX+=d*x; }
    }
  }
  prevGfx.copy(video,0,0,vw,vh,0,0,vw,vh);
  prevGfx.updatePixels();
...
function drawHUD(){
  ...
  text(`cam:${video?'on':'off'}  sway:${nf(swayLP,1,2)}  flip:${flipDir?'ON':'OFF'}`, 20,46);
}
